<?php
	...
	if($button='register' AND $id='signup'){
		...
		*/
		...
		/* versi PDO */
		$query = $conn->prepare("SELECT max(nm_lengkap) AS last FROM admin
								WHERE nm_lengkap LIKE '$today%'");
		$query->execute();
		$data = $query->fetch();
		$lastAdmin = $data['last'];
		...
		*/
			...
			$tujuan = $email;
			$subjek = "Admin PSB SMK XYZ - Password admin";
			$link = "http://localhost/PSBNEW/cfg/dist/aktivasi.php?code=$kode_aktivasi"; /*-- jika sudah hosting, ubah dengan link URL website --*/
			$pesan = "Selamat, Anda terdaftar sebagai pengguna sistem PSB SMK XYZ. Klik link tautan berikut $link untuk aktivasi akun pengguna Anda. Username anda adalah $nextAdmin dan password anda adalah $passwd. Silahkan login menggunakan username dan password tersebut.";
